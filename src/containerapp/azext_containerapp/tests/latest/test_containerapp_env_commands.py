...
class ContainerAppPremiumSkuTest(ScenarioTest):
    ...
    def test_containerapp_env_premium_sku_e2e(self, resource_group):
        ...
        self.cmd(f"az network vnet create -l {location} --address-prefixes '14.0.0.0/21' -g {resource_group} -n {vnet}")
        sub_id = self.cmd(f"az network vnet subnet create --address-prefixes '14.0.0.0/21' -n sub -g {resource_group} --vnet-name {vnet}").get_output_in_json()["id"]
