...
class ContainerAppPremiumSkuTest(ScenarioTest):
    ...
    def test_containerapp_env_premium_sku_e2e(self, resource_group):
        ...
        app3 = self.create_random_name(prefix='app2', length=24)

        location = "centraluseuap"

        self.cmd(f"az network vnet create -l {location} --address-prefixes '14.0.0.0/23' -g {resource_group} -n {vnet}")
        sub_id = self.cmd(f"az network vnet subnet create --address-prefixes '14.0.0.0/23' -n sub -g {resource_group} --vnet-name {vnet}").get_output_in_json()["id"]

        self.cmd(f'containerapp env create -g {resource_group} -n {env} -s {sub_id} --location {location} --plan premium')
        ...
        self.cmd(f"az containerapp create -g {resource_group} --target-port 80 --ingress external --image mcr.microsoft.com/azuredocs/containerapps-helloworld:latest --environment {env} -n {app3} --workload-profile GP3")
        ...
        url = self.cmd(f"az containerapp create -g {resource_group} -n {app3}").get_output_in_json()["properties"]["configuration"]["ingress"]["fqdn"]
        resp = requests.get(url)
        self.assertTrue(resp.ok)

    @AllowLargeResponse(8192)
    @ResourceGroupPreparer(location="eastus")
    def test_containerapp_env_premium_sku_e2e_australia(self, resource_group):
        import requests
